...
fn parse(input: &str) -> Vec<Vec<usize>> {
    input.split('\n')
        .map(|line| line.split('\t'))
        .map(|line| line.map(|entry| entry.parse::<usize>()
                             .expect("should parse"))
             .collect())
        .collect()
}
...
fn checksum() -> usize {
    ...
    for line in sheet {
        checksum += line.iter().max().expect("max should exist") -
            line.iter().min().expect("min should exist");
    }
    checksum
}

fn find_quotient(row: &[usize]) -> Option<usize> {
    for (i, x) in row.iter().enumerate() {
        for y in &row[i+1..] {
            let pair = &[x, y];
            let larger = pair.iter().max().unwrap();
            let smaller = pair.iter().min().unwrap();
            if **larger % **smaller == 0 {
...
fn divisibles_sum() -> usize {
    let sheet = parse(INPUT);
    sheet.iter().map(|line| find_quotient(line)
